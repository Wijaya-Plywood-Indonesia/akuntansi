<x-filament::page>
    ...
    <div class="pos-pro-dashboard min-h-screen -m-8 p-8 bg-gray-100 dark:bg-gray-950 flex flex-col gap-4 lg:gap-6"
        x-data="{ search: @entangle('search') }">


        {{-- MAIN INFO BAR --}}
        <div
            class="flex flex-wrap items-center justify-between bg-white dark:bg-gray-900 px-4 py-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm gap-x-6 gap-y-2">
            <div class="flex flex-wrap items-center gap-6 w-full lg:w-auto">
                <div class="flex items-center gap-2">
                    <span
                        class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">Nota:</span>
                    <input type="text" wire:model.live="no_nota"
                        class="px-2 py-1 bg-gray-50/50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded text-sm font-black text-primary-600 font-mono focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 w-full transition-all" />
                </div>
                ...
                <div class="flex items-center gap-2 flex-grow min-w-[200px]">
                    <span
                        class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">Catatan:</span>
                    <input type="text" wire:model.live="keterangan_nota" placeholder="Tambahkan catatan nota..."
                        class="px-2 py-1 bg-gray-50/50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700 rounded text-sm text-gray-600 dark:text-gray-400 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 w-full placeholder:text-gray-300 dark:placeholder:text-gray-700 transition-all" />
                </div>
                <div class="h-8 w-px bg-gray-100 dark:bg-gray-800 hidden sm:block"></div>
                <div class="flex items-center gap-2">
                    <span
                        class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">Kasir:</span>
                    <span
                        class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ auth()->user()->name }}</span>
                </div>
                <div class="h-8 w-px bg-gray-100 dark:bg-gray-800 hidden sm:block"></div>
                <div class="flex items-center gap-2">
                    <span
            ...
            <div
                class="hidden lg:flex items-center gap-2 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-lg border border-gray-100 dark:border-gray-700">
                <x-heroicon-s-building-storefront class="w-3.5 h-3.5 text-gray-400 dark:text-gray-500" />
                <span
                    class="text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-tight">{{ $namaToko }}</span>
            </div>
        </div>
        ...
        <div class="flex flex-col xl:flex-row gap-4 xl:gap-6">
            ...
            <div class="w-full xl:w-[68%] flex flex-col gap-4 xl:gap-6 order-1">
                ...
                <div class="bg-white dark:bg-gray-900 p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm"
                    @click.outside="$wire.set('showDropdown', false)">
                    <div class="space-y-3">
                        <div class="relative">
                            ...
                            @if ($showDropdown)
                                <div wire:key="search-dropdown-results"
                                    class="absolute inset-x-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto">
                                    @foreach ($searchResults as $barang)
                                        <div wire:key="search-item-{{ $barang->id }}"
                                            wire:click="selectBarang({{ $barang->id }})"
                                            class="px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer flex justify-between items-center border-b border-gray-50 dark:border-gray-700 last:border-0">
                                            <div>
                                                <div class="font-semibold text-sm text-gray-900 dark:text-white">
                                                    {{ $barang->nama_barang }}
                                                    @if ($barang->satuan)
                                                        <span
                                                            class="text-[10px] text-gray-500 font-bold ml-1">({{ $barang->satuan->nama_satuan }})</span>
                                                    @endif
                                                </div>
                                                <div class="text-[10px] text-gray-500 uppercase">{{ $barang->barcode }}
                                                </div>
                                            </div>
                                            <div class="text-right">
                                                <div class="font-bold text-sm text-primary-600">
                                                    Rp{{ number_format($barang->harga_jual) }}</div>
                                                <div class="text-[9px] text-gray-400">Stok:
                                                    {{ number_format($barang->stok_aktual, 2) }}</div>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                            <div wire:key="col-tipe-pelanggan" class="flex flex-col gap-1.5">
                                <label
                                    class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide ml-1">Tipe
                                    Pelanggan</label>
                                <div
                                    class="grid grid-cols-2 gap-1 p-1 bg-gray-100/50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                                    <button type="button" wire:click="toggleMember(0)"
                                        class="py-1.5 rounded-md text-[10px] font-bold uppercase transition-all {{ !$is_member ? 'bg-white dark:bg-gray-700 shadow-sm text-primary-600' : 'text-gray-500 dark:text-gray-400' }}">Umum</button>
                                    <button type="button" wire:click="toggleMember(1)"
                                        class="py-1.5 rounded-md text-[10px] font-bold uppercase transition-all {{ $is_member ? 'bg-white dark:bg-gray-700 shadow-sm text-primary-600' : 'text-gray-500 dark:text-gray-400' }}">Member</button>
                                </div>
                            </div>

                            @if ($is_member)
                                <div wire:key="col-kode-member" class="flex flex-col gap-1.5 relative"
                                    @click.outside="if ($wire.customerResults && $wire.customerResults.length) $wire.set('customerResults', [])">
                                    <label
                                        class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide ml-1">Kode
                                        Member / Cari</label>
                                    <input type="text" wire:model.live.debounce.300ms="kode_member"
                                    ...
                                    @if (!empty($customerResults))
                                        <div wire:key="col-customer-results-dropdown"
                                            class="absolute top-full left-0 right-0 z-50 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                                            @foreach ($customerResults as $res)
                                                <button type="button" wire:click="selectCustomer({{ $res->id }})"
                                                    ...
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-xs font-bold dark:text-white">{{ $res->nama }}</span>
                                                        <span
                                                            class="text-[10px] text-gray-500 dark:text-gray-400">{{ $res->nik }}
                                                            | {{ $res->telepon }}</span>
                                                    </div>
                                                </button>
                                            @endforeach
                            ...
                            <div wire:key="col-customer-info-{{ $is_member ? 'member' : 'umum' }}"
                                class="{{ $is_member ? 'md:col-span-2' : 'md:col-span-3' }} flex flex-col gap-1.5">
                                <label
                                    class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide ml-1">Nama
                                    & Alamat Terpilih</label>
                                <div class="grid grid-cols-2 gap-2">
                                    <input type="text" wire:model.live="nama_customer" placeholder="Nama..."
                                        class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-sm focus:ring-2 focus:ring-primary-500/20 dark:text-white" />
                                    <input type="text" wire:model.live="alamat" placeholder="Alamat..."
                                        class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-sm focus:ring-2 focus:ring-primary-500/20 dark:text-white" />
                                </div>
                            </div>
                        </div>
                ...
                <div
                    ...
                    <div
                        ...
                        <h3 class="text-xs font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest">Item
                            Belanja</h3>
                        <span
                            class="text-[10px] font-black text-primary-600 bg-primary-50 dark:bg-primary-900/40 px-3 py-1 rounded-full uppercase">{{ count($cart) }}
                            Items</span>
                    </div>
                            ...
                            <table class="w-full min-w-[700px] text-left table-fixed border-collapse">
                                <thead class="bg-gray-50/50 dark:bg-gray-800/50">
                                    <tr
                                        ...
                                        <th
                                            class="px-2 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider w-[25%]">
                                            Item</th>
                                        <th
                                            class="px-2 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center w-[22%]">
                                            Qty</th>
                                        <th
                                            class="px-2 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right w-[18%]">
                                            H. Jual</th>
                                        <th
                                            class="px-2 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right w-[16%]">
                                            Potongan</th>
                                        <th
                                            class="px-2 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right w-[16%]">
                                            Total</th>
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                    @foreach ($cart as $id => $item)
                                        <tr wire:key="cart-item-desktop-{{ $id }}"
                                            class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                            <td class="px-2 py-2">
                                                <div
                                                    class="font-medium text-sm text-gray-900 dark:text-white line-clamp-1">
                                                    {{ $item['nama_barang'] }}
                                                    <span class="text-[10px] text-gray-400 font-normal ml-1">•
                                                        Rp{{ number_format($item['harga_awal']) }}</span>
                                                </div>
                                            </td>
                                            <td class="px-2 py-2">
                                                <div class="flex items-center justify-center gap-1.5"
                                                    ...
                                                    <div
                                                        class="flex items-center justify-center bg-gray-100/50 dark:bg-gray-800 rounded-md p-0.5 border border-gray-200 dark:border-gray-700">
                                                        <button wire:click="decrementQty({{ $id }})"
                                                            class="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-primary-600"><x-heroicon-o-minus
                                                                class="w-3 h-3" /></button>
                                                        <input type="text" :value="format(qty)"
                                                            @input="
                                                        ...
                                                        <button wire:click="incrementQty({{ $id }})"
                                                            class="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-primary-600"><x-heroicon-o-plus
                                                                class="w-3 h-3" /></button>
                                                    </div>
                                                    <span
                                                        class="text-xs text-gray-500 font-bold whitespace-nowrap">{{ $item['satuan'] }}</span>
                                                </div>
                                            </td>
                                            <td class="px-2 py-2" x-data="{
                                                ...
                                                <input type="text" :value="format(harga)"
                                                    @input="
                                            ...
                                            <td class="px-2 py-2" x-data="{
                                                ...
                                                <input type="text" :value="format(potongan)"
                                                    @input="
                                                        let raw = $event.target.value.replace(/\D/g, '');
                                    ...
                                    <div class="flex justify-between items-start gap-3">
                                        <div class="flex-grow">
                                            ...
                                            <div class="text-[10px] text-gray-400 font-bold uppercase mt-1">
                                                {{ $item['satuan'] }} • Rp{{ number_format($item['harga_awal']) }}
                                            </div>
                                        </div>
                                        <button wire:click="removeFromCart({{ $id }})"
                                            class="text-gray-300 hover:text-red-500 p-1"><x-heroicon-o-trash
                                                class="w-4 h-4" /></button>
                                    </div>
                                    <div class="grid grid-cols-2 gap-4">
                                        <div class="space-y-1">
                                            <label
                                                class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Qty</label>
                                            <div class="flex items-center bg-gray-50 dark:bg-gray-800 rounded-xl p-1 border border-gray-100 dark:border-gray-700"
                                                ...
                                                <button wire:click="decrementQty({{ $id }})"
                                                    class="w-8 h-8 flex items-center justify-center text-gray-400"><x-heroicon-o-minus
                                                ...
                                                <button wire:click="incrementQty({{ $id }})"
                                                    class="w-8 h-8 flex items-center justify-center text-gray-400"><x-heroicon-o-plus
                                                        class="w-3.5 h-3.5" /></button>
                                            </div>
                                        </div>
                                        <div class="space-y-1" x-data="{
                                            ...
                                            <label
                                                class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">H.
                                                Jual</label>
                                            <input type="text" :value="format(harga)"
                                                @input="
                                                    let raw = $event.target.value.replace(/\D/g, '');
                                        ...
                                        <div class="space-y-1" x-data="{
                                            ...
                                            <label
                                                class="text-[10px] font-black text-red-400 uppercase tracking-widest ml-1">Diskon</label>
                                            <input type="text" :value="format(potongan)"
                                                @input="
                                                    let raw = $event.target.value.replace(/\D/g, '');
                                                    potongan = raw ? parseInt(raw) : 0;
                                        ...
                                        <div class="space-y-1 text-right">
                                            <label
                                                class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Subtotal</label>
                                            <div class="py-1.5 font-black text-sm text-primary-600">
                                                Rp{{ number_format($item['subtotal']) }}</div>
                                        </div>
                                    </div>
                                </div>
                            @empty
                                <div class="py-20 text-center opacity-20"><x-heroicon-o-shopping-bag
                                        class="w-12 h-12 mx-auto mb-2" /><span
                                        class="text-xs font-black uppercase">Kosong</span></div>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            <div id="checkout-section" class="w-full xl:w-[32%] order-2">
                <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-lg flex flex-col overflow-hidden"
                    ...
                    <div
                        class="p-4 lg:p-5 bg-primary-600 dark:bg-black text-white relative overflow-hidden shrink-0 transition-colors duration-300">
                        <div class="relative z-10">
                            <span
                                class="text-[9px] font-bold uppercase tracking-widest text-primary-100 dark:text-primary-500 block mb-1">Grand
                                Total</span>
                            <div class="flex items-baseline gap-1">
                                <span class="text-sm font-bold opacity-50 dark:opacity-30">Rp</span>
                                <span
                                    class="text-2xl lg:text-3xl font-black tracking-tight leading-none">{{ number_format($this->total) }}</span>
                            </div>
                        </div>
                    ...
                    <div class="p-4 lg:p-5 space-y-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                            <label
                                ...
                                Pembayaran</label>
                            <textarea wire:model.live="keterangan_pembayaran" rows="2" placeholder="Catatan pembayaran..."
                        ...
                        <div class="flex flex-col gap-1.5">
                            <label
                                class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block ml-1">Pengiriman</label>
                            <select wire:model.live="metode_pengiriman"
                                class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-sm focus:ring-2 focus:ring-primary-500/10 cursor-pointer transition-all dark:text-white">
                        ...
                        @if ($metode_pengiriman === 'DIKIRIM')
                            <div wire:key="shipping-fields-container" class="space-y-3 pt-2">
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="flex flex-col gap-1.5">
                                        <label
                                            class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block ml-1">Kendaraan</label>
                                        <input type="text" wire:model.live="kendaraan" placeholder="Kendaraan..."
                                            class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-sm focus:ring-2 focus:ring-primary-500/10 dark:text-white" />
                                    </div>
                                    <div class="flex flex-col gap-1.5">
                                        <label
                                            class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block ml-1">Plat
                                            Kendaraan</label>
                                        <input type="text" wire:model.live="plat_kendaraan" placeholder="Plat..."
                                            class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-sm focus:ring-2 focus:ring-primary-500/10 dark:text-white" />
                                    </div>
                                </div>
                                <div class="flex flex-col gap-1.5">
                                    <label
                                        class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block ml-1">Nama
                                        Sopir</label>
                                    <input type="text" wire:model.live="nama_sopir" placeholder="Nama Sopir..."
                                        class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-sm focus:ring-2 focus:ring-primary-500/10 dark:text-white" />
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/20">
                        <div class="flex gap-2">
                            ...
                            <button wire:click="resetPos"
                                class="py-2.5 px-4 py-1 btn-danger text-white text-sm font-bold  tracking-wide transition-all rounded-lg">
                                Reset
                            </button>
                        </div>
                    </div>
