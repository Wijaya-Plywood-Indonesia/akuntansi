<x-filament-panels::page>
    ...
    @push('styles')
        ...
        <link
            href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;600&family=DM+Sans:wght@300;400;500;600&display=swap"
            rel="stylesheet">
        ...
        <style>
            ...
            .tree-wrapper {
                font-family: var(--tree-font-body);
            ...
            .tree-induk-header:hover {
                background: var(--tree-hover);
            }

            .tree-induk-header[aria-expanded="true"] {
                background: var(--tree-active);
            }
            ...
            .badge-debet {
                background: var(--badge-debet-bg);
                color: var(--badge-debet-txt);
            }

            .badge-kredit {
                background: var(--badge-kredit-bg);
                color: var(--badge-kredit-txt);
            }
            ...
            /* Badge jumlah barang terhubung (di anak akun & sub anak akun) */
            .barang-count-pill {
                display: inline-flex;
                align-items: center;
                gap: 4px;
            ...
            .tree-children-wrap {
                display: none;
                border-top: 1.5px solid var(--tree-border);
            }

            .tree-children-wrap.open {
                display: block;
            }
            ...
            .tree-node-row {
                ...
                border-radius: 9px;
                cursor: default;
                transition: background 0.12s, border-left 0.12s, padding-left 0.12s;
                position: relative;
                border-left: 2.5px solid transparent;
            }

            .tree-node-row.clickable {
                cursor: pointer;
            }
            ...
            /* Level colors */
            .level-1 .tree-node-code {
                background: rgba(8, 145, 178, 0.1);
                color: var(--lvl-1-accent);
            }

            .level-1>.tree-node-row.clickable:hover,
            .level-1>.tree-node-row.open-row {
                border-left-color: var(--lvl-1-accent);
            }

            .level-1 .tree-chevron {
                background: rgba(8, 145, 178, 0.1);
                color: var(--lvl-1-accent);
            }

            .level-2 .tree-node-code {
                background: rgba(124, 58, 237, 0.1);
                color: var(--lvl-2-accent);
            }

            .level-2>.tree-node-row.clickable:hover,
            .level-2>.tree-node-row.open-row {
                border-left-color: var(--lvl-2-accent);
            }

            .level-2 .tree-chevron {
                background: rgba(124, 58, 237, 0.1);
                color: var(--lvl-2-accent);
            }

            .level-3 .tree-node-code {
                background: rgba(5, 150, 105, 0.1);
                color: var(--lvl-3-accent);
            }

            .level-3>.tree-node-row.clickable:hover,
            .level-3>.tree-node-row.open-row {
                border-left-color: var(--lvl-3-accent);
            }

            .level-3 .tree-chevron {
                background: rgba(5, 150, 105, 0.1);
                color: var(--lvl-3-accent);
            }

            .level-4 .tree-node-code {
                background: rgba(217, 119, 6, 0.1);
                color: var(--lvl-4-accent);
            }

            .level-4>.tree-node-row.clickable:hover,
            .level-4>.tree-node-row.open-row {
                border-left-color: var(--lvl-4-accent);
            }

            .level-4 .tree-chevron {
                background: rgba(217, 119, 6, 0.1);
                color: var(--lvl-4-accent);
            }
            ...
            .barang-group-title {
                ...
                font-weight: 600;
                margin-bottom: 8px;
                color: var(--tree-text-secondary, #5a6080);
                text-transform: uppercase;
                letter-spacing: 0.04em;
            }

            .barang-list-box {
                border: 1px solid var(--tree-border, #e4e7f0);
                border-radius: 10px;
                overflow: hidden;
                margin-bottom: 18px;
            }
            ...
            .barang-list-row+.barang-list-row {
            ...
            .barang-list-stok {
                ...
                font-size: 12px;
                font-weight: 600;
                color: var(--tree-text-primary, #1a1d2e);
                flex-shrink: 0;
                white-space: nowrap;
            }

            /* Stok positif = hijau, nol/netral = abu-abu muted, negatif = merah */
            .barang-list-stok.positif {
                color: #16a34a;
            }

            .barang-list-stok.negatif {
                color: #dc2626;
            }

            .barang-list-stok.nol {
                color: var(--tree-text-muted, #9aa0bb);
            }
    ...
    {{-- ============================================================
     wire:ignore DI SINI: mencegah Livewire re-render/morph ulang
     tree pas modal dibuka (yang cuma butuh update selectedSubAkunId).
     Tree state (expand/collapse) dikelola manual via vanilla JS,
     jadi kalau Livewire ikut morph area ini, semua state expand
     ke-reset balik ke kondisi awal server-render.
     ============================================================ --}}
    <div class="tree-wrapper" id="treeWrapper" wire:ignore>
        ...
        <div class="tree-legend">
            <span class="legend-item"><span class="legend-dot" style="background:var(--lvl-0-accent)"></span>Induk
                Akun</span>
            <span class="legend-item"><span class="legend-dot" style="background:var(--lvl-1-accent)"></span>Anak
                Akun</span>
            <span class="legend-item"><span class="legend-dot" style="background:var(--lvl-2-accent)"></span>Sub
                Anak</span>
            <span class="legend-item"><span class="legend-dot" style="background:var(--lvl-3-accent)"></span>Sub-Sub
                Anak</span>
            <span class="legend-item"><span class="legend-dot" style="background:var(--lvl-leaf-accent)"></span>Detail
                Akun</span>
            &nbsp;
        ...
        <div class="tree-container" id="treeContainer">
            @foreach ($indukAkuns as $induk)
                ...
                <div class="tree-induk-card" id="induk-{{ $induk->id }}">
                    <div class="tree-induk-header" aria-expanded="false" onclick="toggleInduk(this)"
                        data-id="{{ $induk->id }}"
                        ...
                        <div class="tree-meta">
                            @if ($induk->saldo_normal)
                                <span
                                    class="badge badge-{{ $induk->saldo_normal }}">{{ strtoupper(substr($induk->saldo_normal, 0, 2)) }}</span>
                            @endif
                            @if (($induk->barang_count_total ?? 0) > 0)
                                <span class="barang-count-pill" title="Total barang di induk akun ini">
        ...
        <div class="tree-empty" id="treeEmpty">
            <svg width="40" height="40" fill="none" stroke="currentColor" stroke-width="1.5"
                viewBox="0 0 24 24" style="margin: 0 auto 12px; color: var(--tree-text-muted)">
                <circle cx="11" cy="11" r="8" />
                <path d="m21 21-4.35-4.35" />
            </svg>
    ...
    {{-- ============================================================
     MODAL: Barang terkait dengan sub anak akun yang diklik.
     DI LUAR wire:ignore, supaya selalu di-update Livewire tiap
     kali openBarangModal() dipanggil.
     Stok ditampilkan pakai accessor stok_matrix dari model Barang,
     dengan warna: positif = hijau, nol = netral, negatif = merah.
     ============================================================ --}}
    <x-filament::modal id="barang-sub-akun-modal" width="2xl">
        <x-slot name="heading">
            Barang Terkait
            @if ($this->selectedSubAkun)
                <span style="font-weight:400; color:var(--tree-text-muted,#9aa0bb);">
                    — {{ $this->selectedSubAkun->kode_sub_anak_akun }}
                    {{ $this->selectedSubAkun->nama_sub_anak_akun }}
                </span>
            @endif
        </x-slot>

        @php $barang = $this->barangTerkait; @endphp

        @if ($barang['utama']->isEmpty() && $barang['pendapatan']->isEmpty() && $barang['hpp']->isEmpty())
            <div class="barang-empty-state">
                Belum ada barang yang terhubung ke akun ini.
            </div>
        @else
            @foreach ([
        'utama' => 'Sebagai Akun Utama',
        'pendapatan' => 'Sebagai Akun Pendapatan',
        'hpp' => 'Sebagai Akun HPP',
    ] as $key => $label)
                @if ($barang[$key]->isNotEmpty())
                    <div class="barang-group-title">{{ $label }} ({{ $barang[$key]->count() }})</div>
                    <div class="barang-list-box">
                        @foreach ($barang[$key] as $b)
                            @php
                                $stok = $b->stok_matrix;
                                $stokClass = $stok > 0 ? 'positif' : ($stok < 0 ? 'negatif' : 'nol');
                            @endphp
