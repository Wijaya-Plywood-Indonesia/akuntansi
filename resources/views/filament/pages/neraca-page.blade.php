<x-filament-panels::page>
    ...
    <div class="flex items-center justify-between mb-2">
        ...
        @if ($this->periodeValid() && $this->jumlahPeriode() > 0)
            <button wire:click="exportExcel" wire:loading.attr="disabled" wire:target="exportExcel"
                ...
                <svg wire:loading wire:target="exportExcel" class="animate-spin w-4 h-4 flex-shrink-0" fill="none"
                    viewBox="0 0 24 24">
                ...
                <svg wire:loading.remove wire:target="exportExcel" class="w-4 h-4 flex-shrink-0" fill="none"
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-end gap-6">
            ...
            <div class="flex-shrink-0 pb-1">
                @if (!$this->periodeValid())
                    <div
                        class="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700
                ...
                @else
                    <div
                        class="flex items-center gap-2 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-700
                                text-primary-700 dark:text-primary-400 rounded-xl px-4 py-3 text-sm font-medium">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <span><strong>{{ $this->jumlahPeriode() }}</strong> data {{ $jenisFilter }}an
                            ditampilkan</span>
                    </div>
                @endif
            </div>
    ...
    @elseif($this->jumlahPeriode() === 0)
        <div class="text-center py-16">
            <svg class="mx-auto w-14 h-14 text-gray-300 dark:text-gray-600 mb-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
        ...
        <div class="space-y-8">
            @foreach ($this->neracaMulti as $key => $neraca)
                @php
                    ...
                    $fmtM3 = fn($v) => is_numeric($v) && $v != 0
                        ? number_format((float) $v, 2, ',', '.') . ' m³'
                        : null;
                ...
                <div
                    ...
                    <div
                        ...
                        <div>
                            <p
                                class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-0.5">
                                INA TELUR
                            </p>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-1.5">
                                <button type="button" @click="$dispatch('neraca-expand-all')"
                                ...
                                <button type="button" @click="$dispatch('neraca-collapse-all')"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold
                                       bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400
                                       border border-gray-300 dark:border-gray-600 hover:border-primary-400
                                       hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 9V5m0 0H5m4 0L4 4m11 5V5m0 0h4m-4 0l5-5M9 15v4m0 0H5m4 0l-5 5m11-5v4m0 0h4m-4 0l5 5" />
                                    </svg>
                                    Collapse All
                                </button>
                            </div>
                            ...
                            <button wire:click="$toggle('tampilkanSaldoNol')"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm border-collapse" style="min-width:700px">
                            <thead>
                                ...
                                <tr
                                    class="bg-gray-50 dark:bg-gray-700/40 text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                                    <th
                                        class="border border-gray-100 dark:border-gray-700 py-1.5 px-4 text-left font-semibold w-[32%]">
                                        Akun</th>
                                    <th
                                        class="border border-gray-100 dark:border-gray-700 py-1.5 px-3 text-right font-semibold w-[7%]">
                                        Qty</th>
                                    <th
                                        class="border border-gray-100 dark:border-gray-700 py-1.5 px-3 text-right font-semibold w-[6%]">
                                        m³</th>
                                    <th
                                        class="border border-gray-100 dark:border-gray-700 py-1.5 px-4 text-right font-semibold w-[11%]">
                                        Nilai (Rp)</th>
                                    <th
                                        class="border border-gray-100 dark:border-gray-700 py-1.5 px-4 text-left font-semibold w-[32%]">
                                        Akun</th>
                            ...
                            transition-colors">
                                        ...
                                        {{-- ── AKTIVA ── --}}
                                        <td class="border border-gray-100 dark:border-gray-700 py-3 px-4
                                ...
                                {{ $aRow && $isItem && !empty($aRow['sub_akuns']) ? 'cursor-pointer hover:bg-gray-50/85 dark:hover:bg-gray-700/35 transition-colors select-none' : '' }}"
                                            @if ($aRow && $isItem && !empty($aRow['sub_akuns'])) x-data="{ open: false }" @click="open = !open" @neraca-expand-all.window="open = true" @neraca-collapse-all.window="open = false" @endif>
                                            @if ($aRow)
                                                <span
                                                    class="{{ $isSub ? 'text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400' : '' }}">
                                                    @if ($isItem)
                                                        <span class="inline-flex items-center gap-2">
                                                            @if (!empty($aRow['sub_akuns']))
                                                            ...
                                                            @if (!empty($aRow['kode']))
                                                                <span
                                                                    class="font-mono text-xs text-amber-600 dark:text-amber-400
                                                            ...
                                                            @endif
                                                            <span
                                                                class="font-medium text-sm">{{ $aRow['label'] }}</span>
                                                        </span>

                                                        @if (!empty($aRow['sub_akuns']))
                                                            <div x-show="open" x-collapse @click.stop
                                                                class="mt-2.5 space-y-1.5 pl-5 border-l border-gray-200 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
                                                                @foreach ($aRow['sub_akuns'] as $sub)
                                                                    @if (($sub['nilai'] ?? 0) != 0 || $tampilkanSaldoNol)
                                                                        <div
                                                                            class="flex items-center justify-between gap-4 py-0.5 border-b border-gray-50 dark:border-gray-800 last:border-b-0">
                                                                            <span
                                                                                class="font-mono text-[10px] text-amber-600/80 bg-amber-50 dark:bg-amber-950/20 px-1 rounded">{{ $sub['kode'] }}</span>
                                                                            <span
                                                                                class="truncate flex-1 text-left">{{ $sub['nama'] }}</span>
                                                                            <span
                                                                                class="font-mono text-xs text-gray-600 dark:text-gray-400">{{ $fmt($sub['nilai']) }}</span>
                                                                        </div>
                                                                    @endif
                                                                @endforeach
                                                            </div>
                                        ...
                                        <td
                                            class="border border-gray-100 dark:border-gray-700 py-2 px-3 text-right tabular-nums whitespace-nowrap">
                                        ...
                                        <td
                                            class="border border-gray-100 dark:border-gray-700 py-2 px-4 text-right tabular-nums whitespace-nowrap
                                {{ $isTot ? 'font-semibold text-gray-800 dark:text-gray-100' : 'text-gray-700 dark:text-gray-300' }}">
                                            @if ($aRow && isset($aRow['nilai']) && !$isHdr && !$isSub)
                                                <span
                                                    class="{{ $isTot ? 'border-t border-b border-gray-400 dark:border-gray-500 px-1' : '' }}">
                                                    {{ $fmt($aRow['nilai']) }}
                                                </span>
                                            @endif
                                        </td>
                                ...
                                {{ $pRow && $isItem && !empty($pRow['sub_akuns']) ? 'cursor-pointer hover:bg-gray-50/85 dark:hover:bg-gray-700/35 transition-colors select-none' : '' }}"
                                            @if ($pRow && $isItem && !empty($pRow['sub_akuns'])) x-data="{ open: false }" @click="open = !open" @neraca-expand-all.window="open = true" @neraca-collapse-all.window="open = false" @endif>
                                            @if ($pRow)
                                                <span
                                                    class="{{ $isSub ? 'text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400' : '' }}">
                                                    @if ($isItem)
                                                        <span class="inline-flex items-center gap-2">
                                                            @if (!empty($pRow['sub_akuns']))
                                                                <span class="text-gray-400 dark:text-gray-500">
                                                                    <svg class="w-3.5 h-3.5 transition-transform"
                                                                        :class="open ? 'rotate-90' : ''" fill="none"
                                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                            ...
                                                            @endif
                                                            <span
                                                                class="font-medium text-sm">{{ $pRow['label'] }}</span>
                                                        </span>

                                                        @if (!empty($pRow['sub_akuns']))
                                                            <div x-show="open" x-collapse @click.stop
                                                                class="mt-2.5 space-y-1.5 pl-5 border-l border-gray-200 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
                                                                @foreach ($pRow['sub_akuns'] as $sub)
                                                                    @if (($sub['nilai'] ?? 0) != 0 || $tampilkanSaldoNol)
                                                                        <div
                                                                            class="flex items-center justify-between gap-4 py-0.5 border-b border-gray-50 dark:border-gray-800 last:border-b-0">
                                                                            <span
                                ...
                                <tr class="bg-gray-800 dark:bg-gray-900 text-white">
                                    ...
                                    <td class="border border-gray-700 px-4 py-3 text-right tabular-nums">
                                        <span
                                            class="font-bold text-base border-t-2 border-b-4 border-double border-white px-2">
                    ...
                    {{-- Card Footer --}}
                    <div
                        class="px-6 py-3 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-100 dark:border-gray-700
